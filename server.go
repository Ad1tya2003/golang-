...
var jwtKey = []byte("everyoneshallknowpain")
...
func main() {
    ...
    db, err = sql.Open("mysql", "lord1:lord1@tcp(localhost:3306)/testdb2?parseTime=true")
...
func loginHandler(c *gin.Context) {
    ...
    c.JSON(http.StatusOK, gin.H{"token": tokenString, "userID": userID})
...
func requestOTPHandler(c *gin.Context) {
    ...
    err := db.QueryRow("SELECT COUNT(*) FROM otps WHERE user_id=? AND created_at > ?", otpReq.UserID, time.Now().Add(-blockDuration)).Scan(&attemptCount)

    if err == nil && attemptCount >= otpRequestLimit {

        c.JSON(http.StatusBadRequest, gin.H{"error": "Too many OTP requests. Try again after 15 mins."})

        return

    }

    err = db.QueryRow("SELECT created_at FROM otps WHERE user_id=? ORDER BY id DESC LIMIT 1", otpReq.UserID).Scan(&lastCreatedAt)

    if err == nil && time.Since(lastCreatedAt) < otpExpiration {

        c.JSON(http.StatusBadRequest, gin.H{"error": "OTP request too soon. Try again after 15 seconds."})
    ...
    _, err = db.Exec("INSERT INTO otps (user_id, otp, created_at) VALUES (?, ?, ?)", otpReq.UserID, otp, time.Now())
...
func verifyOTPHandler(c *gin.Context) {
    ...
    if otpReq.UserID == 0 || otpReq.OTP == "" {

        c.JSON(http.StatusBadRequest, gin.H{"error": "UserID and OTP are required"})
    ...
    err := db.QueryRow("SELECT otp, created_at FROM otps WHERE user_id=? ORDER BY id DESC LIMIT 1", otpReq.UserID).Scan(&storedOTP, &createdAt)
    ...
    if time.Since(createdAt) > otpExpiration {
